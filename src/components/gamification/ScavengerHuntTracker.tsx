...
export function ScavengerHuntTracker() {
    const { foundEmbers, totalEmbers, isComplete, isClaimed } = useScavengerHunt();
    const [showModal, setShowModal] = useState(false);
    const [isExpanded, setIsExpanded] = useState(true);

    // If no embers found OR already claimed (and modal closed), don't show the tracker
    if (foundEmbers.length === 0) return null;
    if (isClaimed && !showModal) return null;
                        ...
                        {isExpanded && (
                            ...
                            >
                                ...
                                <button
                                    ...
                                    aria-label="Close scavenger hunt tracker"
                                >
                                    <X size={14} />
                                </button>
                            </motion.div>
